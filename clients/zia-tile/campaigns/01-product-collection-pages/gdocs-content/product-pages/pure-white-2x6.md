---
title: "Pure White 2x6 - Product Page"
google_doc_id: "12KuVATj1PfzeDUfpxEr9vJGA5i7xcL21EIH6IS_hj6Q"
page_type: product
download_date: 2026-05-12
---

**URL:** <https://ziatile.com/products/pure-white-2x6-zellige-tile> 

**Title Tag:** Pure White 2x6 Bejmat Zellige | Zia Tile

**Meta Description:** Shop Zia's Pure White 2x6 Bejmat zellige. A handcrafted Moroccan tile suited for kitchen backsplashes, bathroom walls, and shower enclosures.

  

## Zellige 

# Pure White 2x6

\[INSERT CART MODULE HERE\]

  

Zia's Pure White 2x6 zellige draws from a centuries-old Moroccan tradition still practiced today by skilled artisans. The same techniques once used to compose the decorative geometric and floral mosaics of grand palaces and riads now produce this elongated 2x6 Bejmat format, well-suited to residential and commercial projects worldwide. The rectangular proportion supports a range of layout options, including running bond, vertical stack, and herringbone, each of which carries the tile's inherent variation differently across the field.

  

Expect inherent variation in size, color, and tone, alongside the chips, pits, and crazing that define this handcrafted Moroccan art form. These features are the source of zellige's perfectly imperfect character. Images shown reflect a sample of the tile and may not capture the full range of dynamic glazes present across a complete order.

  
  

\[INSERT IMAGE: SIZE/THICKNESS\]

**Order Details + Installation**

We recommend a 15–20% overage for each project. This additional quantity allows the installer to curate the installation and account for tiles that fall outside the desired range. 

  

Setting zellige is closer to masonry than to ordinary tile work, given how much the thickness, edge, and size of each piece can vary. Installers sort, soak, and adjust tiles individually rather than working to a fixed grid, with extra attention paid to the layout pattern, given the rectangular format. Plan for two to three times the installation window of a precision-cut tile, and use wedge spacers to keep spacing consistent and edges protected throughout the field.

  

  
  
  

## About — \[ACCORDION\]

### Inherent Variation

  - Our Pure White 2x6 is a soft white Bejmat zellige boasting dynamic glazes that read differently across the surface depending on lighting conditions.
  - Each handcrafted Bejmat tile carries the marks of its firing, including shifts in surface texture, edge character, and depth, and this innate variation defines the line's character.
  - Expect variation in color, shape, and thickness, alongside characteristic chips, pits, and crazing.

### Preparation and Curation

  - Mix all boxes in the shipment before beginning the project to ensure a balanced distribution of color and tone.
  - Lay out the full installation on a flat surface ahead of setting, so the layout pattern and tile variation read the way you want.
  - Any chips, pits, and crazing should be evenly dispersed across the field.
  - A 15–20% overage on all zellige tile orders gives you enough material to curate the mix to your preference. For an even more uniform look, we recommend 25% overage.

### Installation and Finishing

  - Installers must always wear protective face and eye coverings when working with zellige, thinset, sealants, and grout in accordance with California Prop 65.
  - While traditional trim and edge pieces do not exist for zellige, we recommend bringing the drywall flush with the finished surface of the tile or finishing the project with mitered edges or Schluter strips.
  - Always seal Pure White zellige 2x6 according to our installation instructions, whether in dry spaces or in areas exposed to moisture, such as pools, spas, and showers.
  - Reach out to our team directly at info@ziatile.com or 310-844-1170 with any material or installation questions.

  

*⚠ WARNING: Cancer and Reproductive Harm.* [*Learn more about Proposition 65 Warnings.*](https://www.ziatile.com/proposition-65-warnings)

  

  

## Tile Usage — \[ACCORDION\]

Our Pure White 2x6 Bejmat zellige offers a versatile rectangular format suited to residential and commercial projects alike. The 2x6 proportion supports horizontal running bond, vertical stack, and herringbone layouts.

### Residential Usage

  - **Interior Floors – High Traffic:** Pure White 2x6 Bejmats perform well across high-traffic areas, including kitchens and entryways.
  - **Interior Floors – Low Traffic:** A reliable choice for low-traffic settings such as guest bathrooms and guest bedrooms.
  - **Exterior Floors – Freeze or Thaw:** Zellige is not suitable for outdoor installation in a freeze/thaw climate.
  - **Exterior Floors – Non Freeze or Thaw:** Well-suited to outdoor surfaces in climates that do not experience snow or ice.
  - **Interior Walls:** Suitable for any interior wall application, including kitchen backsplashes and bathroom walls.
  - **Exterior Walls:** Pure White 2x6 zellige may be installed on exterior walls in non-freeze/thaw climates.
  - **Shower Walls + Floors:** Pure White 2x6 brings a clean, dimensional surface to shower walls, shower floors, and steam showers in this Bejmat format. Slip resistance can be increased by increasing grout joint size to a minimum of ⅛" and/or applying an anti-slip treatment after installation is complete. The tile and grout should always be sealed.
  - **Pools + Spas:** Often specified in and around pools, in both saltwater and chlorine environments. The tile should always be sealed according to our installation instructions.
  - **Fireplace Surround:** Pure White 2x6 produces a refined fireplace surround, though the tiles are not approved for use inside the firebox.
  - **Radiant Heating Compatible:** Compatible with radiant heating and heated floor systems.
  - **Sealing Required:** Always seal Pure White zellige 2x6 according to our installation instructions, whether in dry spaces or in areas exposed to moisture, such as pools, spas, and showers.

### Commercial Usage

  - **Interior Floors – High Traffic:** Pure White 2x6 Bejmats perform well across high-traffic commercial settings, such as restaurant kitchens, hotel lobbies, and entryways. If installed on a flooring surface in a wet area, **slip resistance** can be increased by sizing grout joints to a minimum of ⅛" and/or applying an **anti-slip treatment** after installation is complete.
  - **Interior Floors – Low Traffic:** This Bejmat format performs well across lower-traffic commercial interiors such as private offices and restrooms.
  - **Exterior Floors – Freeze or Thaw:** Pure White 2x6 zellige is not suitable for outdoor installation in a freeze/thaw climate.
  - **Exterior Floors – Non Freeze or Thaw:** Well-suited to outdoor commercial surfaces in climates that do not experience snow or ice.
  - **Interior Walls:** Suitable for commercial interior walls, including commercial kitchen backsplashes and bar walls.
  - **Exterior Walls:** Pure White 2x6 zellige may be installed on exterior commercial walls in non-freeze/thaw climates.
  - **Shower Walls + Floors:** Pure White 2x6 brings a clean, dimensional surface to commercial shower walls, shower floors, and steam showers. Slip resistance can be increased by increasing grout joint size to a minimum of ⅛" and/or applying an anti-slip treatment after installation is complete. The tile and grout should always be sealed.
  - **Pools + Spas:** Often specified for commercial pools, in both saltwater and chlorine environments. The tile should always be sealed according to our installation instructions.
  - **Fireplace Surround:** Pure White 2x6 produces a refined commercial fireplace surround, though the tiles are not approved for use inside the firebox.
  - **Radiant Heating Compatible:** Compatible with commercial radiant heating systems, including heated floor elements.
  - **Sealing Required:** Always seal Pure White zellige 2x6 according to our installation instructions, whether in dry spaces or in areas exposed to moisture, such as pools, spas, and showers.

  

## How It's Made — \[ACCORDION\]

Each Pure White 2x6 is made by Moroccan artisans whose technique has been passed down across generations, producing tiles with both lasting quality and innate variation.

  

The process begins with raw clay drawn from local Moroccan hillsides. Artisans soak the clay in water and filter it for impurities until the material becomes soft and workable, then press it into rectangular molds and set it out under the Moroccan sun to dry. Once the clay has firmed up, each tile is flattened and refined by hand.

  

The tiles are dipped individually in glaze and placed into an earthen kiln, where heat moves unevenly through the chamber. That uneven distribution is what produces the dynamic glazes and tonal range specific to the collection. Each Pure White 2x6 carries the marks of the artisan's day at the kiln.

## Order & Shipping — \[ACCORDION\]

### Sample Orders

  - Our in-stock Pure White 2x6 zellige samples typically ship from our Los Angeles warehouse within two business days of order receipt.
  - Select the "Add Sample to Cart" button to add a Pure White 2x6 sample to your cart.
  - Each client may order up to ten samples total, with a limit of four per each unique tile.
  - We offer five complimentary samples; additional samples begin at $3 each.
  - Your sample order will ship via FedEx or UPS Ground.

### Full Orders

  - We will ship the entire order together once all tiles are in stock.
  - All standard full orders ship throughout the United States and Canada via a third-party LTL carrier service. Pallets will be delivered curbside.
  - LTL carriers provide estimated delivery ranges and will contact you to schedule a specific delivery window once the shipment arrives at their local terminal.
  - We deliver to nearly any location worldwide. Clients in Alaska, Hawaii, Canada, or international locations should email our team at info@ziatile.com for a custom shipping quote.

### Warehouse Pickup & Support

  - Free full order pickup is offered at our Los Angeles area warehouse to help local customers save on shipping costs.
  - For additional questions regarding the Pure White zellige 2x6 order, please reach out to us at info@ziatile.com or 310-844-1170.
  - Our team will reach out when the order is ready so to schedule a pickup appointment. Order pickups are available only with a confirmed appointment, Monday through Friday from 8 am–3 pm.

  

  

## Installation Guide — \[ACCORDION\] 

\[KEEP\]

## 

## Frequently Asked Questions — \[ACCORDION\]  

### What is a Bejmat zellige tile?

A Bejmat zellige tile is a traditional Moroccan zellige format defined by its elongated rectangular shape. The 2x6 proportion supports a wider range of layout patterns than a square format. Running bond, vertical stack, and herringbone each distribute the tile's inherent variation differently across the surface. Zia's Pure White Bejmat zellige is handmade by Moroccan artisans using the same traditional methods as each tile in the zellige collection.

### Can I use Pure White Bejmat zellige in a shower?

Yes, you can install Pure White Bejmat zellige on shower walls and shower floors. Sealing is required for this application. To address slip resistance on shower floors, increase grout joint size to a minimum of ⅛" and/or apply an anti-slip treatment after installation is complete. Refer to the Zellige Installation Guide for sealer recommendations.

### What materials pair well with Pure White Bejmat zellige?

Zia's Pure White Bejmat zellige tile pairs well with natural and refined materials that balance its texture and reflectivity. Pure white zellige complements wood cabinetry, marble or quartz surfaces, and warm metal finishes such as brass, allowing the tile's variation and glaze to remain the focal point of the space.

### Why do some Pure White 2x6 tiles have fine lines on the surface?

Fine lines on the glazed surface of Pure White 2x6 zellige, known as crazing, occur naturally during the earthen kiln firing process. These characteristic marks, along with occasional chips, pits, and crazing, are innate to handmade Moroccan tile and do not affect the structural integrity of the tile.

### Can I mix Pure White 2x6 zellige with other colors in the collection?

Yes, you can mix Pure White 2x6 zellige with other colors in Zia's handcrafted zellige collection. Artisans use the same traditional Moroccan methods for each shade, allowing you to combine Pure White 2x6 tiles with other tones to create a custom installation that showcases the full range of available dynamic glazes.

### Is Pure White Bejmat zellige high maintenance?

Pure White Bejmat zellige is not high maintenance when properly installed and sealed. The glazed surface can be maintained with a pH-neutral cleaner such as Miracle Sealants pH Neutral Tile and Stone Cleaner. In wet areas, re-seal at least every one to two years to maintain protection.
