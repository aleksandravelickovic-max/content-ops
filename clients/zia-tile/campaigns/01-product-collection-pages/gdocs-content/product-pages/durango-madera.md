---
title: "Durango Madera"
google_doc_id: "1dnC3Fg76ZASAoZIQhseEA-kU-R8v4iqbzNJ3eCL91Aw"
page_type: product
download_date: 2026-05-12
---

**URL:** https://ziatile.com/products/durango-madera

**Meta title**: Durango + Madera Terra Cotta Tile | Zia Tile
**Meta description**: Explore Zia Tile Durango + Madera Cotto, a handcrafted Mexican terra cotta tile defined by rich earthy tones and natural variation

**Keywords:** Durango + Madera cotto, brown terra cotta tile, Specialty shape cotto tile

## Cotto

# Durango + Madera

\[INSERT CART MODULE HERE\]



Zia's Durango + Madera is a handmade terra cotta tile produced in San Miguel de Allende, Mexico, combining our Durango specialty shape with the Madera colorway. The finished installation reads as a minimalist architectural composition, with the rust-brown tones of the Madera glaze carrying across the curved geometry of the Durango format. Installed across a sunlit patio or interior floor, the shape interlocks into a continuous patterned field where the sculptural profile of each piece builds the visual rhythm of the surface.



Each Durango + Madera tile includes inherent variation in tone, shape, and surface texture, along with the soft, irregular edges that define unglazed cotto. The images shown represent the tile's general appearance and may not capture the full range of dynamic tones present in a complete order.



\[INSERT IMAGE: SIZE/THICKNESS\]

## Details



We recommend a 15% overage to curate the installation and account for any cuts needed. Because the Durango is a specialty shape, the additional cuts required during layout often warrant increasing overage beyond the standard minimum; consider 20% or more before placing your order.



Cotto installation requires a skilled installer experienced with porous clay materials and specialty shape layouts. Each Durango piece varies in thickness, edge profile, and tone, and the interlocking geometry depends on consistent orientation across the field. Plan the installation around a full dry layout, and protect the surface with sealing before and after grouting.



## About

### Color and Surface

  - Our Durango + Madera is a rust-brown terra cotta tile with a matte, unglazed surface that grounds the Durango specialty shape in the earthy tonal range of natural Mexican clay.
  - Each handmade brown terra cotta tile remains wholly unique, and this innate variation defines the line's charm.
  - Expect natural variation in tone, shape, thickness, and surface texture across the layout, along with the soft, irregular edges inherent to unglazed cotto. Chips, pits, and crazing may appear as part of the firing process.

### Preparation and Curation

  - Mix the contents of each box in your shipment before setting to distribute tonal and surface variation evenly across the installation.
  - Complete a full dry layout before committing to placement. The Durango specialty shape only resolves correctly when each piece is oriented as intended, so confirm the alignment of the interlocking geometry before setting begins.
  - Disperse any tonal and surface variation evenly across the layout rather than allowing darker or lighter pieces to cluster in isolated areas.
  - We suggest a 15% overage on all cotto orders. For the Durango specialty shape, the additional cuts required warrant increasing overage to 20% or more.

### Installation and Finishing

  - All installers must wear protective face and eye coverings when working with cotto, thinset, sealants, and grout in accordance with California Prop 65.
  - For finishing, bring the drywall flush with the finished tile surface, miter exposed edges at 45 degrees, or complete the edge with Schluter strips.
  - Seal Durango + Madera with 511 Porous Plus before and after grouting. For interior applications, finish with Fila Matte Wax as a final coat. For exterior applications, apply a second coat of 511 Porous Plus in place of wax.
  - Use 1/8" tile spacers and plan the layout carefully. The Durango geometry depends on precise orientation during the dry layout stage to resolve across the full field.
  - You may reach out to our team directly at info@ziatile.com with any specific installation questions.



*⚠ WARNING: Cancer and Reproductive Harm.* [*Learn more about Proposition 65 Warnings.*](https://www.ziatile.com/proposition-65-warnings)



## Tile Usage

Our Durango + Madera cotto suits both residential and commercial projects. The specialty shape cotto tile works across floor and wall applications, bringing the rust-brown Madera palette and architectural Durango geometry to kitchen floors, outdoor patios, feature walls, and hospitality interiors in both freeze/thaw and non-freeze/thaw climates.

### Residential Usage

  - **Interior Floors – High Traffic:** Handles daily use in kitchens, entryways, hallways, and primary bathrooms. In wet floor applications, slip resistance can be increased by sizing grout joints to a minimum of 1/8" and/or applying an anti-slip product after installation. Cotto DCOF: 0.98.
  - **Interior Floors – Low Traffic:** Suits guest bedrooms, secondary bathrooms, and living spaces where the Durango geometry reads as an architectural element across the floor.
  - **Exterior Floors – Freeze or Thaw:** Suitable for outdoor installations in freeze/thaw climates. Apply a second coat of 511 Porous Plus in place of wax for exterior use.
  - **Exterior Floors – Non Freeze or Thaw:** Suitable for outdoor patios, courtyards, and walkways in climates without snow or ice.
  - **Interior Walls:** Suited to feature walls and architectural surfaces where the sculptural shape and rust-brown palette establish the design direction of the room.
  - **Exterior Walls:** Suitable for exterior wall applications in any climate, including retaining walls and architectural facades.
  - **Shower Walls + Floors:** Suited to shower walls, shower floors, and steam showers. Sealing is required for this use. Slip resistance can be increased by sizing grout joints to a minimum of 1/8" and/or applying an anti-slip product after installation.
  - **Pools + Spas:** Not suitable for pools or spas.
  - **Fireplace Surround:** Suited to decorative fireplace surrounds. These tiles are not for use inside the firebox.
  - **Radiant Heating Compatible:** Compatible with radiant heating systems.
  - **Sealing Required:** Sealing is required before and after grouting using 511 Porous Plus. Indoors, finish with Fila Matte Wax. Outdoors, apply a second coat of 511 Porous Plus. Refer to the Zia Tile Installation Guide for detailed instructions.

### Commercial Usage

  - **Interior Floors – High Traffic:** Suited to hotel lobbies, retail floors, restaurant entries, and office corridors where the Durango geometry defines the surface. In wet floor applications, slip resistance can be increased by sizing grout joints to a minimum of 1/8" and/or applying an anti-slip product after installation. Cotto DCOF: 0.98.
  - **Interior Floors – Low Traffic:** Appropriate for offices, meeting rooms, and boutique interiors where the sculptural format adds a distinct architectural layer.
  - **Exterior Floors – Freeze or Thaw:** Suitable for commercial exterior installations in freeze/thaw climates. Apply a second coat of 511 Porous Plus in place of wax for exterior use.
  - **Exterior Floors – Non Freeze or Thaw:** Suitable for commercial exterior surfaces such as restaurant patios and hotel courtyards in climates without snow or ice.
  - **Interior Walls:** Suited to hospitality features, restaurant walls, and retail installations where the Durango shape and rust-brown palette establish a signature surface.
  - **Exterior Walls:** Suitable for commercial exterior wall systems in any climate, including facades and retaining structures.
  - **Shower Walls + Floors:** Suited to commercial shower walls, shower floors, and steam showers. Sealing is required for this use. Slip resistance can be increased by sizing grout joints to a minimum of 1/8" and/or applying an anti-slip product after installation.
  - **Pools + Spas:** Not suitable for pools or spas.
  - **Fireplace Surround:** Suited to decorative fireplace surrounds in lobbies and hospitality spaces. Not for use inside a firebox.
  - **Radiant Heating Compatible:** Compatible with commercial radiant heating systems.
  - **Sealing Required:** Sealing is required before and after grouting using 511 Porous Plus. Indoors, finish with Fila Matte Wax. Outdoors, apply a second coat of 511 Porous Plus. Refer to the Zia Tile Installation Guide for detailed instructions.



## How It's Made

Our Durango + Madera begins with natural Mexican clay drawn from the hills around San Miguel de Allende, a region that has produced terra cotta for centuries. The clay is refined through soaking and filtering to achieve a workable consistency that preserves the mineral composition responsible for the rust-brown tones of the Madera colorway.



Once prepared, the clay is hand-pressed into the Durango specialty mold that defines each tile's sculptural form. The curved geometry is established at the pressing stage and requires careful handling throughout the process. Each tile is then left to dry slowly over one to two weeks, allowing moisture to release gradually and stabilizing the structure before firing.



The tiles are fired in traditional kilns at high temperatures, where heat distribution shifts naturally across the chamber. This temperature variation produces subtle differences in tone and surface density across each Durango + Madera piece, which become part of the finished character rather than an inconsistency to be corrected. The unglazed finish preserves the direct connection between the clay body and the fired surface, and the rust-brown tones deepen further as the surface develops a natural patina with use.



## Order & Shipping

### Sample Orders

  - Our in-stock Durango + Madera cotto samples typically ship from our Los Angeles warehouse within two business days of order receipt.
  - Select the "Add Sample to Cart" button to add Durango + Madera to your cart.
  - Each client may order up to ten samples total, with a limit of four per each unique tile.
  - We offer five complimentary samples; additional samples begin at $3 each.
  - Sample orders travel via FedEx or UPS ground.

### Full Orders

  - Full orders ship throughout the US and Canada via LTL carrier with curbside pallet delivery.
  - All tiles in your order ship together once everything is in stock.
  - Your LTL carrier will provide an estimated delivery range and contact you to confirm a delivery window once the shipment reaches their local terminal.
  - We deliver to almost any location around the world. Clients in Alaska, Hawaii, Canada, or international locations should contact our team for a custom shipping quote.

### Warehouse Pickup & Support

  - We offer free full-order pickup at our Los Angeles area warehouse to help local customers save on shipping costs.
  - We will contact you once your order is ready to schedule a pickup appointment. A confirmed appointment is required for all warehouse pickups.
  - For additional questions regarding your Durango + Madera order, please reach out to us at <info@ziatile.com>.



## Installation Guide

\[KEEP\]



## Frequently Asked Questions

### What is the Durango + Madera cotto made of?

The Durango + Madera cotto is made from natural Mexican clay sourced near San Miguel de Allende, Mexico. The clay is refined, mixed with water, and hand-pressed into the Durango specialty mold before drying for one to two weeks. Each tile is then kiln-fired, and the heat variation and mineral interaction during firing produce the rust-brown tonal range and surface character of the finished tile. Chips, pits, and crazing develop during this process and are inherent to the material.

### What paint colors pair well with Durango + Madera?

Durango + Madera pairs with paint colors that either balance or contrast the rust-brown palette. Warm off-whites and cream tones create a neutral surrounding field that lets the sculptural shape lead. Olive, sage, and muted greens align with the earthy tonal range. Deeper colors such as charcoal, navy, or black introduce stronger contrast and shift the overall composition. The choice depends on whether the tile should lead the room or integrate into a broader palette.

### Can Durango + Madera be used in a kitchen?

Yes, Durango + Madera cotto can be used in a kitchen on both floors and walls. Proper installation and sealing are required to manage the natural porosity of unglazed terra cotta. Apply 511 Porous Plus before and after grouting to protect the surface, and maintain a minimum 1/8" grout joint. In areas exposed to grease or moisture, such as behind a range, ongoing sealing prevents absorption and staining.

### How do I clean terra cotta tile?

To clean terra cotta tile, use a pH-neutral cleaner with warm water and avoid acidic or abrasive products. Sweep or vacuum regularly to remove abrasive debris that can wear the surface over time. Clean with a damp cloth or mop rather than excess water, and dry the surface afterward to prevent residue buildup. Periodic resealing is required to maintain long-term performance.

### How does the Durango specialty shape install?

The Durango specialty shape is a curved, sculptural format that interlocks with adjacent pieces to build an architectural composition across the surface. Because the geometry depends on consistent orientation, a full dry layout before installation is essential to confirm how the shape resolves across the field. The shape also requires more cuts than a standard square or rectangle, which is why increasing overage beyond the standard 15% is recommended for Durango orders.

### Can I mix Durango + Madera with other cotto colorways?

Yes, you can mix Durango + Madera with other cotto colorways in Zia's collection. Each colorway is produced using the same clay body and firing process, so tiles share consistent surface character across the range. The rust-brown Madera works particularly well alongside lighter cotto colorways such as Blanco or the warmer traditional terra cotta tones, where the tonal contrast creates a layered composition across the sculptural Durango field.
