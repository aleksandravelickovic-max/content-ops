---
sku: durango-fired-earth
material: cotto
url: https://ziatile.com/products/durango-fired-earth
title_tag: Durango + Fired Earth Terra Cotta Tile | Zia Tile
meta_description: Shop Zia's Durango + Fired Earth Cotto. A handcrafted Mexican terra cotta specialty set in wood-ash-fired warm clay — suited for floors, outdoor patios, and feature walls in any climate.
keywords: Durango Fired Earth cotto tile, specialty shape terra cotta tile, geometric cotto tile set
...
Zia's Durango + Fired Earth is a handmade terra cotta tile produced in San Miguel de Allende, Mexico, in the Durango specialty shape — a four-tile set that interlocks to build an abstract geometric pattern across the installation, with each tile measuring 10" × 10.5". The Fired Earth colorway sits in the mid-range of the classic Cotto palette: a warm terra cotta tone with surface patterning created by wood ash applied during firing. Where the ash contacts the clay, it produces varied, textured coloration across the warm clay base, so no two tiles read identically within the set. The Durango geometry — which resolves only once all four tiles of the set are oriented and placed correctly — combines with the ash-fired surface to make the installed field more complex with each additional set added. The unglazed surface deepens into patina with wear, bringing beauty to the material's aging process. Images shown represent a sample of this tile and do not display every potential variation.
