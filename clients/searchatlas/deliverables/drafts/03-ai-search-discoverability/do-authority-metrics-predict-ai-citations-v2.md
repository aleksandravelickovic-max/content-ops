...
- v2 changes from v1: added meta title + description; added byline; defined "win rate" at first use; corrected Domain Power definition (backlink trust IS an input, per products/domain-power.md); rewrote product section to connect capabilities to the co-mention-competition + per-platform mechanisms (voice fix); de-duplicated the per-platform sentence between body and FAQ; softened list-item refrains.
- Domain Power recalibration claim deliberately omitted (internal-only, not in public report).
- Publish date: TBD at publication (study itself published 2025-11-10). datePublished must be set in CMS for Article schema.
- Internal-link slot: add a live LLM Visibility tool page URL at publish (only the research report URL is available now).
- Length ~1,750 words. Slightly below 1,800-2,500 target; not padded per CLAUDE.md no-filler rule. Editor decision.
- Kept the first H2 in exact-query form for AEO snippet capture despite judge note; rewrote its opening so it advances past the intro rather than restating it.
- Title/body use "Domain Authority/DA" and "DR" deliberately: these refer to the third-party Moz/Ahrefs metrics that ARE the article's subject, not to SearchAtlas's metric. STYLE-SYSTEM §3.2 bans "domain authority" only as a name for SearchAtlas's own metric (which is correctly called Domain Power throughout). Approved exception for query-matching.
- Post-gate polish (v2, after PASS): defined "co-mention competition" at first use; anchored Google/YouTube figures to "the study's platform-level breakdown"; reworded the LLM Visibility output to documented terms (share of voice vs co-cited competitors) to clear the last claims-grounding flag.
-->
