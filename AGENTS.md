...
```
zia-content-ops/
├── AGENTS.md                             # This file — project rules (always in context)
├── README.md / README.html               # Project overview (HTML version for stakeholders)
├── content-toolkit/                      # Agents, commands, and settings (symlinked as .Codex/)
...
├── .Codex -> content-toolkit/           # Symlink — Codex auto-discovers from here
├── universal-rules/
...
| Makana Charters | `clients/makana-charters/STYLE-SYSTEM.md` | Active — Native Hawaiian family-owned Na Pali Coast boat tour operator, Kauai HI; 200-year family legacy; never guarantee wildlife sightings or sea cave access; post-migration SEO recovery focus (CSM: Wendy Alinor) |
...
| Find Self Storage | `clients/find-self-storage/STYLE-SYSTEM.md` | Active — self-storage comparison marketplace; 10,000+ facilities; consumer audience only; marketplace framing critical (never "our facilities"); no confirmed price-match guarantee |
...
- `/parallel-pieces <client> <type> <material> <count|list>` — **the scale lever.** Fans out N pieces as parallel Task() sub-agents inside one Codex session. Same RR pattern. Use this instead of running `/run-piece` in a loop.
